...
// Exact basenames that are almost always machine-generated (dependency lock
// files and similar). Mirrors a subset of GitHub linguist's generated-file
// heuristics.
...
// Filename suffixes that signal generated code (minified bundles, source maps,
// protobuf/grpc output, codegen conventions, snapshots, etc.).
...
// Path fragments whose presence marks the whole tree as generated/vendored.
const GENERATED_PATH_FRAGMENTS = [
	'/node_modules/',
	'/vendor/',
	'/__generated__/',
	'/__snapshots__/',
...
// Matches in-file markers conventionally placed at the top of generated files:
// the Go convention ("// Code generated ... DO NOT EDIT.") and the broader
// "@generated" / "DO NOT EDIT" markers used across ecosystems.
const GENERATED_CONTENT_MARKER =
	/@generated\b|DO NOT EDIT|Code generated by|This file was automatically generated|autogenerated|auto-generated/i

// Heuristically decides whether a file in a branch diff is generated code so it
...
function isGeneratedFile(filename: string, lines: { text: string }[]): boolean {
	...
	if (GENERATED_PATH_FRAGMENTS.some((fragment) => normalizedPath.includes(fragment))) {
		return true
	}
...
function App() {
	...
	// Code-body scroll position captured per window when it collapses, and the set
	// of windows awaiting scroll restoration after expanding. Used by
	// toggleWindowCollapse and the restore layout effect.
	const collapsedScrollRef = useRef<Map<string, number>>(new Map())
	const pendingExpandScrollRef = useRef<Set<string>>(new Set())
	...
	// After a window expands, restore its code body's scroll position so it returns
	// to exactly the same scope of content it showed before collapsing. The guard
	// keeps this a no-op for the far more frequent unrelated openFiles updates.
	useLayoutEffect(() => {
		if (pendingExpandScrollRef.current.size === 0) {
			return
		}
		for (const windowID of pendingExpandScrollRef.current) {
			const body = workspaceRef.current?.querySelector(
				`[data-window-id="${windowID}"] .file-code-scroll`,
			)
			const scrollTop = collapsedScrollRef.current.get(windowID)
			if (body instanceof HTMLElement && scrollTop !== undefined) {
				body.scrollTop = scrollTop
			}
		}
		pendingExpandScrollRef.current.clear()
	}, [openFiles])
	...
	function createDiffWindow(fileDiff: BranchFileDiff, base: string, compare: string, index: number) {
		...
		const pendingWindow = {
			...
			// Auto-collapse generated files (lockfiles, codegen, minified bundles,
			// vendored trees), mirroring GitHub's diff view. The header toggle still
			// lets the user expand them.
			collapsed: isGeneratedFile(fileDiff.filename, fileDiff.lines),
		}
	...
	// Toggles a window between full and header-only. Collapsing captures the code
	// body's current scroll position; expanding queues it for restoration (applied
	// in a layout effect once the body is visible again) so the window comes back
	// to exactly the same size and scroll as before.
	function toggleWindowCollapse(windowID: string) {
		const target = openFiles.find((fileWindow) => fileWindow.id === windowID)
		if (target === undefined) {
			return
		}
		...
		if (!target.collapsed) {
			...
			if (body instanceof HTMLElement) {
				collapsedScrollRef.current.set(windowID, body.scrollTop)
			}
		} else {
			pendingExpandScrollRef.current.add(windowID)
		}

		setOpenFiles((current) =>
			current.map((fileWindow) =>
				fileWindow.id === windowID
					? { ...fileWindow, collapsed: !fileWindow.collapsed }
					: fileWindow,
			),
		)
	}
						...
						return (
							<section
								...
								className={
									[
										'file-window',
										isActive ? 'file-window-active' : '',
										fileWindow.collapsed ? 'file-window-collapsed' : '',
									]
										.filter(Boolean)
										.join(' ')
								}
								...
								{fileWindow.state === 'loading' ? (
									...
									<>
										...
										>
											...
											<div className="file-window-controls">
												<button
													type="button"
													className="file-window-control-button file-window-collapse-button"
													aria-label={
														fileWindow.collapsed
															? `Expand ${fileWindow.filename}`
															: `Collapse ${fileWindow.filename}`
													}
													aria-expanded={!fileWindow.collapsed}
													onClick={() => toggleWindowCollapse(fileWindow.id)}
												>
													{fileWindow.collapsed ? '▸' : '▾'}
												</button>
												<button
													type="button"
													className="file-window-control-button file-window-close-button"
